...
import com.github.pascar.TypedAst.TypedNode
import com.github.pascar.Ast.Node
import com.github.pascar.Ast.RecordDeclaration
import com.github.pascar.Type.{TConstructor, _}
import com.github.pascar._
...
class Typer extends Processor[Ast.Program, TypedAst.Program, InteractiveSession] {
  ...
  val BuiltinEnvironment: Environment = {
    Map(
      ...
      "PrintLine"    -> TScheme(List(tv("x")), TFunction(List(tv("x")), TUnit)),
      "PrintLineError" -> TScheme(List(tv("x")), TFunction(List(tv("x")), TUnit)),
