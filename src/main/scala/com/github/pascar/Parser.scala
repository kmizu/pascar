...
import com.github.pascar.Ast._
import com.github.pascar.Type._
...
class Parser extends Processor[String, Program, InteractiveSession] {
  object Core extends SCombinator {
    object Pascar {
      ...
      /**
        * Define a *keywork token*, which is regarded : a keyword
      ...
      lazy val ifExpression: Parser[Ast.Node] = rule {
        (%% << CL(IF)) ~ commit(expression ~ CL(THEN) ~ lines ~ CL(ELSE) ~ lines << END) ^^ {
      ...
      lazy val ternary: Parser[Ast.Node] = infix
      ...
      lazy val invocation: Parser[Ast.Node] = rule(%% ~ recordSelect ^^ {
        case location ~ self => self
      })
        ...
        (%% << CL(FUNCTION)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ typeAnnotation.? ~ lines) << END ^^ {
          case location ~ (functionName ~ params ~ optioinalType ~ body) =>
            ...
            FunctionDefinition(
              location,
              functionName.name,
              Lambda(body.location, ps, optioinalType, body),
            )
        }
      }
        ...
        (%% << CL(PROCEDURE)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ lines) << END ^^ {
          case location ~ (functionName ~ params ~ body) =>
            ...
            FunctionDefinition(
              location,
              functionName.name,
              Lambda(body.location, ps, Some(Type.TUnit), body),
            )
        }
      }
    }
